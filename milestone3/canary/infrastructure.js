...
AWS.provisionAWSInstance(aws_params, function(instanceInfo){
	var inventory = "\n"+"canary1 ansible_ssh_host=" + instanceInfo.ipAddr + " ansible_ssh_user=ubuntu ansible_ssh_private_key_file=/home/ubuntu/devops-m3.pem";
  fs.appendFileSync("inventory", inventory);
  var json = JSON.stringify(instanceInfo);
  client.lpush(['canaryServers', json], function(err, reply){
    if(err) throw err;
    client.quit();
  });
});